...
// --- MIDDLEWARE ---
app.use(cors());
// Use express.json() with a larger limit to handle the full application state
...
// All other requests serve the main HTML file, allowing React Router to handle the path.
app.get('*', (req, res) => {
...
// --- START THE SERVER ---
app.listen(PORT, () => {
    console.log(`Servidor ELIMENTAL escuchando en http://localhost:${PORT}`);
    console.log('Para acceder desde otro dispositivo en la misma red, use la IP de su NAS (ej. http://192.168.1.100:3001)');
});
